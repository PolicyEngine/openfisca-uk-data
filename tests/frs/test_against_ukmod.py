...
from openfisca_uk_data.datasets.frs.base_frs.model_input_variables import get_input_variables
...
calc_functions = [partial(get_quantile, q=q) for q in np.linspace(0, 1, 11)] + [lambda values: values.sum(), lambda values: (values > 0).sum()]
metric_to_func = {m: f for m, f in zip(metrics, calc_functions)}

@pytest.mark.parametrize("variable,UKMOD_variable,metric", map(lambda x: (*x[0], x[1]), product(PAIRS.items(), metrics)))
def test_distribution_parameter_close_to_UKMOD(variable, UKMOD_variable, metric):
	...
	assert result == target or -MAX_RELATIVE_ERROR < result / target - 1 < MAX_RELATIVE_ERROR